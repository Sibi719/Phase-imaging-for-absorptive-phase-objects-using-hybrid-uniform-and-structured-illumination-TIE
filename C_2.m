...
[uni_rec_ph, deld]=uniform(dpix,d,lambda,Hsize,phase,k,Ioz,freq_sq,noise,T);
uni_rec_ph=uni_rec_ph+ abs(min(min(uni_rec_ph)));
% figure('units','normalized','outerposition',[0 0 1 1]);
% imagesc(x,y,deld); 
% colormap(jet);
% colorbar
...
%%%Phi_x
[rphi_x,phi_x, deld,delmxd,delqxd]=phix(dpix,d,lambda,Hsize,phase,k,Ioz,Imxz,Iqxz,freq_sq,noise,fm,u,n,cut,T);
rphi_x=rphi_x+ abs(min(min(rphi_x)));
...
%%%Phi_y
[rphi_y,phi_y, deld,delmyd,delqyd]=phiy(dpix,d,lambda,Hsize,phase,k,Ioz,Imyz,Iqyz,freq_sq,noise,fm,u,n,cut,T);
rphi_y=rphi_y+ abs(min(min(rphi_y)));
...
com_rec_ph=com_rec_ph+ abs(min(min(com_rec_ph)));
figure('units','normalized','outerposition',[0 0 1 1]);
...
hybrid_image = hybrid_image + abs(min(min(hybrid_image)));
...
function [uni_rec_ph, deld]=uniform(dpix,d,lambda,Hsize,phase,k,Ioz,freq_sq,noise,T)

Hcrop=sqrt(T).*exp(1j.*phase);
Iod=FresProp(dpix,d,lambda,Hsize,Hcrop);
deld=(Iod-(Ioz.*T))+noise;
...
function [rphi_x,phi_x, deld,delmxd,delqxd]=phix(dpix,d,lambda,Hsize,phase,k,Ioz,Imxz,Iqxz,freq_sq,noise,fm,u,n,cut,T)

Hcrop=sqrt(Imxz.*T).*exp(1j.*phase);
Imxd=FresProp(dpix,d,lambda,Hsize,Hcrop);
Hcrop=sqrt(Iqxz.*T).*exp(1j.*phase);
Iqxd=FresProp(dpix,d,lambda,Hsize,Hcrop);
Hcrop=sqrt(Ioz.*T).*exp(1j.*phase);
Iod= FresProp(dpix,d,lambda,Hsize,Hcrop);
...
function [rphi_y,phi_y, deld,delmyd,delqyd]=phiy(dpix,d,lambda,Hsize,phase,k,Ioz,Imyz,Iqyz,freq_sq,noise,fm,u,n,cut,T)

Hcrop=sqrt(Imyz.*T).*exp(1j.*phase);
Imyd=FresProp(dpix,d,lambda,Hsize,Hcrop);
Hcrop=sqrt(Iqyz.*T).*exp(1j.*phase);
Iqyd=FresProp(dpix,d,lambda,Hsize,Hcrop);
Hcrop=sqrt(Ioz.*T).*exp(1j.*phase);
Iod= FresProp(dpix,d,lambda,Hsize,Hcrop);
